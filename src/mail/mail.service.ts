...
export class MailService {
  ...
  async sendOrder(order: Order, itemGetter: ItemGetter) {
    const positions = await Promise.all(order.positions.map(async (position, i) => {
        const item = await itemGetter.findById(position.itemId);
        return `
          <table>
            <tbody>
              <tr><td>Товар:</td><td><a href="https://hleb365.ru/product/${position.itemId}">${item ? item.name : `Товар ${i+1}` }</a></td></tr>
